...
class arch(nn.Module):
    ...
    def __init__(self, input_dim=3, dim=128, scale_factor=4, scale_ratio=0.2, negative_slope=0.2):
        ...
        self.block1 = ResNetBlock(kernel_size=3)
        self.block2 = ResNetBlock(kernel_size=3)
        self.block3 = ResNetBlock(kernel_size=3)
        self.block4 = ResNetBlock(kernel_size=3)
        self.conv3 = torch.nn.Conv2d(64, 16, 3, 1, 1)
        self.conv4 = torch.nn.Conv2d(16, 3, 1, 1, 0)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        LR_feat = F.leaky_relu(self.conv1(input), negative_slope=0.2)
